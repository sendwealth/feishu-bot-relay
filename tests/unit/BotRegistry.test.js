...
describe('BotRegistry', () => {
  ...
  describe('register()', () => {
    ...
    test('应该拒绝无效的机器人配置', () => {
      expect(() => {
        registry.register({});
      }).toThrow('Invalid bot configuration');
    });
  ...
  describe('get()', () => {
    ...
    test('应该返回null当机器人不存在时', () => {
      const bot = registry.get('non_existent');
      expect(bot).toBeNull();
    });
  });
  ...
  describe('updateStatus()', () => {
    test('应该更新机器人状态', () => {
      registry.register(mockBots.botA);
      registry.updateStatus(mockBots.botA.botId, 'inactive');
    ...
    test('应该返回false当机器人不存在时', () => {
      const result = registry.updateStatus('non_existent', 'active');
      expect(result).toBe(false);
    });
  });
